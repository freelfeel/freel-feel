...
def makeImage(numPics = 20):
   with open("eqns.txt", 'w') as eqnsFile:
      for i in range(numPics):
         ...
         image.save("../static/img" + str(i) + ".png", "PNG")
         print("img" + str(i) + ".png   --> SAVED")

def last_day_tally():
  conn=sqlite3.connect('../FFpir.db')
  curs=conn.cursor()
  for row in curs.execute("select count(*) from pir where timestamp between datetime('now','-2 day') and datetime('now','-1 day');"):
    count = str(row[0])
    #conn.close()
  return int(count)


makeImage(last_day_tally())
